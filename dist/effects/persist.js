...
function sanitizeState(raw) {
    ...
    const num = (v, fallback, min, max) => typeof v === 'number' && isFinite(v) ? Math.min(max, Math.max(min, v)) : fallback;
    const color = (v, fallback) => typeof v === 'string' && CSS_COLOR_RE.test(v.trim()) ? v.trim() : fallback;
    ...
    s.lineHeight = LH_ALLOWED.has(raw.lineHeight) ? raw.lineHeight : s.lineHeight;
    s.letterSpacing = LS_ALLOWED.has(raw.letterSpacing) ? raw.letterSpacing : s.letterSpacing;
    s.textAlign = TA_ALLOWED.has(raw.textAlign) ? raw.textAlign : s.textAlign;
